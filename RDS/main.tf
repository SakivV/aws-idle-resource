...
resource "aws_lambda_function" "idle_rds_instance_function" {
...
resource "aws_cloudwatch_log_group" "idle_rds_instance_function_lg" {
  name = "/aws/lambda/${aws_lambda_function.idle_rds_instance_function.function_name}"
...
resource "aws_cloudwatch_event_target" "invoke_lambda" {
  ...
  arn       = aws_lambda_function.idle_rds_instance_function.arn
}

resource "aws_lambda_permission" "allow_cloudwatch_to_invoke_lambda" {
  statement_id  = "AllowExecutionFromCloudWatch"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.idle_rds_instance_function.function_name
